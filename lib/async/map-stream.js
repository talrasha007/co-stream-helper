  ...
  _transform(data, encoding, done) {
    ...
    this._mapper(data).then(mappedData => {
      if (this._opt.isFilter) {
        ...
        if (this._opt.flatten) {
          if (mappedData.forEach) {
            mappedData.forEach(item => item !== null && item !== undefined && this.push(item));
          } else {
            throw new Error('flatMap mapper function should return iterable value.');
          }
        } else {
...
module.exports = (mapper, opt) => new MapStream(mapper, opt);